...
final class OllamaProvider extends Provider
{
    ...
    public function __construct(
        ...
        $this->baseUrl = rtrim($baseUrl ?? config('prism.providers.ollama.url', 'http://localhost:11434'), '/');
        $this->apiKey = $apiKey ?? config('prism.providers.ollama.api_key', '');
    ...
    public function text(TextRequest $request): TextResponse
    {
        ...
        $finishReason = $this->mapFinishReason(data_get($data, 'done_reason', 'stop'));
        ...
        return new TextResponse(
            ...
            usage: $usage,
            meta: new Meta(
                id: data_get($data, 'id', uniqid('ollama_')),
                model: $request->model()
            ),
            ...
            return match (true) {
                ...
                $message instanceof ToolResultMessage => [
                    ...
                    'content' => is_string($message->toolResults[0]->result ?? null)
                        ...
                        : json_encode($message->toolResults[0]->result ?? '', JSON_UNESCAPED_UNICODE),
                ],
                default => [
                    'role' => 'user',
    ...
    protected function makeRequest(array $body): Response
    {
        ...
        $response = $http->post($this->baseUrl.'/api/chat', $body);
    ...
    {
        return new Usage(
            promptTokens: (int) ($data['prompt_eval_count'] ?? $data['prompt_tokens'] ?? 0),
            completionTokens: (int) ($data['eval_count'] ?? $data['completion_tokens'] ?? 0),
        );
    }
